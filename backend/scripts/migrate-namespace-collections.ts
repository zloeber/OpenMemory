...
async function analyzeCollection(
    ...
    while (hasMore) {
        ...
        for (const point of result.points) {
            stats.totalVectors++;
            const userId = (point.payload?.user_id as string) || 'default';
...
async function createNamespaceCollection(
    client: QdrantClient,
...
async function migrateNamespace(
    ...
    while (hasMore) {
        ...
        for (const point of result.points) {
            ...
            currentBatch.push(point as VectorPoint);
...
`);
    ...
    for (const [userId] of stats.namespaces) {
        try {
            const migrated = await migrateNamespace(
                client,
    ...
    if (verified && options.deleteSource) {
        ...
        console.log(`   To delete it, run: curl -X DELETE ${env.qdrant_url}/collections/${options.sourceCollection}`);
    }
